...
% the standard second-order 9pt discretization of the Laplacian.  The
% optimal time step of dx^2/6 and correction terms are used.
%
% USAGE:
...
while (t < t_final)
  ...
  % update solution
  log_u = log(u);
  u = u + dt*(L_9pt*u) - dt*u.*log_u ...
    + 0.5*dt^2*( -L_5pt*(u.*log_u) - (L_5pt*u).*log_u ...
                + u.*(log_u.^2) - (L_5pt*u) + u.*log_u );
