...
for i = 1:length(grid_sizes)
  ...
  if (use_saved_data == 1)
    ...
    theta = atan2(Y,X);
    phi = sqrt(X.^2+Y.^2) - 0.6*(1+0.5*sin(5*theta));
    ...
    % solve diffusion equation using forward Euler with OTS-DC
    disp('---------------------------');
    disp('  Forward Euler OTS-DC');
    disp('---------------------------');
...
axis([-1 1 -1 1 -2 2]);
view(127.5, 55);
filename = sprintf('diffusion_eqn_2d_starfish_domain_soln.%s', print_suffix);
format_str = sprintf('-d%s', print_format);
print([fig_dir, '/', filename], format_str);
...
xlabel('x'); ylabel('y'); 
axis square
filename = sprintf('diffusion_eqn_2d_starfish_domain_error.%s', print_suffix);
format_str = sprintf('-d%s', print_format);
print([fig_dir, '/', filename], format_str);
