...
% 1d constant-coefficient wave equation without a source term.
%  
% Kevin T. Chu
% 2008 August
...
use_saved_data = 0;
data_dir = 'data-wave_eqn_1d-no_src';
...
% physical parameters
c = 0.5;  % wave speed
use_source_term = 0;  % do not use source term
...
text(2000,1e-3,order_str);

axis([100 10000 1e-15 1e0]);
xlabel('N');
ylabel('L^\infty Error');
filename = sprintf('wave_eqn_1d_no_src_error_vs_N.%s', print_suffix);
...
xlabel('x');
%title('KPY Solution')
filename = sprintf('wave_eqn_1d_no_src_KPY_soln.%s', print_suffix);
format_str = sprintf('-d%s',print_format);
print([fig_dir, '/', filename], format_str);
