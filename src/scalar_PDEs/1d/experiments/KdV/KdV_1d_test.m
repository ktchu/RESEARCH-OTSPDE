...
max_c = max(c_1, c_2);
plot_axes = [x_lo x_hi -0.4*max_c 0.6*max_c];
...
% set initial data
u_init = c_1/2*(sech(sqrt(c_1)/2*x)).^2 ...
       + c_2/2*(sech(sqrt(c_2)/2*(x+1))).^2;
...
while (t < t_final)
  ...
  plot(x,u,'bo');
  hold on;
  u_exact = c_1/2*(sech(sqrt(c_1)/2*(x-c_1*t))).^2 ...
          + c_2/2*(sech(sqrt(c_2)/2*(x+1-c_2*t))).^2;
  plot(x,u_exact,'r');
...
%  rhs = u + dt*(-6*u.*(D1*u)) + dt^2/2*correction_term;
  ...
  correction_term = -3*D4*(u.^2) + 6*D1*(u.*(D3*u)) ...
                  + 18*D1*(u.*(D1*(u.^2)));
  rhs = u + dt*(-3*D1*(u.^2)) + dt^2/2*correction_term;
