...
% equation without a source term when solved using the DuFort-Frankel 
% scheme.
%  
% Kevin T. Chu
% 2009 May
...
use_saved_data = 1;
data_dir = 'data-diffusion_eqn_1d_no_src_DF';
...
text(140,1e-3,order_str);

axis([10 1000 1e-12 1e0]);
set(gca, 'ytick', [1e-12, 1e-10, 1e-8, 1e-6, 1e-4, 1e-2, 1e0]);
xlabel('N');
ylabel('L^\infty Error');
filename = sprintf('diffusion_eqn_1d_no_src_DF_error_vs_N.%s', print_suffix);
...
text(4e-6,10,order_str);

axis([1e-10 1e-2 1e-4 1e2]);
set(gca, 'xtick', [1e-10, 1e-8, 1e-6, 1e-4, 1e-2]);
xlabel('L^\infty Error');
ylabel('Compute Time (s)');
filename = sprintf('diffusion_eqn_1d_no_src_DF_comp_time.%s', print_suffix);
format_str = sprintf('-d%s', print_format);
print([fig_dir, '/', filename], format_str);
...
filename = sprintf('diffusion_eqn_1d_no_src_DF_OTS_soln.%s', print_suffix);
format_str = sprintf('-d%s', print_format);
print([fig_dir, '/', filename], format_str);
...
filename = sprintf('diffusion_eqn_1d_no_src_DF_soln.%s', print_suffix);
format_str = sprintf('-d%s', print_format);
print([fig_dir, '/', filename], format_str);
