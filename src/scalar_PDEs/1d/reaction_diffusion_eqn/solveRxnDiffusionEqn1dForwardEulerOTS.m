%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% solveRxnDiffusionEqn1dForwardEulerOTS() computes the solutions of the 
...
% approximation to the Laplacian.  The optimal time step of dt = dx^2/6
% and correction terms are used to boost the order of accuracy of the 
% solution.
%
% USAGE:
%   function [u, u_exact, x, timing_data] = ...
%     solveRxnDiffusionEqn1dForwardEulerOTS(dx, ...
%                                           t_final, ...
%                                           debug_on, timing_on)
%
% Arguments:
...
while (t < t_final)
  ...
  u_corr = 0.5*dt^2*(L*(-u.^3+u.^2+2*u) + (-3*u.^2+2*u+2).*u_t);
