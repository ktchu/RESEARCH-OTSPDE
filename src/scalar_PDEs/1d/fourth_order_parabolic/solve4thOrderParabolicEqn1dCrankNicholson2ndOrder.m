%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% solve4thOrderParabolicEqn1dCrankNicholson2ndOrder() computes the solutions 
...
% second-order central difference approximation to the bilaplacian.  
%
% USAGE:
%   function [u, u_exact, x, timing_data] = ...
%     solve4thOrderParabolicEqn1dCrankNicholson2ndOrder(dx, dt, ...
%                                                       t_init, t_final, ...
%                                                       debug_on, timing_on)
%
% Arguments:
...
% check arguments
if (nargin < 4)
  error('solve4thOrderParabolicEqn1dCrankNicholson2ndOrder: missing arguments');
end
if (nargin < 5)
  debug_on = 0;
end
...
N = 2/dx - 1;
x = -1+dx:dx:1-dx; x = x'; 
x_bc_minus = -1-dx:dx:-1; x_bc_minus = x_bc_minus';
x_bc_plus = 1:dx:1+dx; x_bc_plus = x_bc_plus';
...
% construct 2nd-order Biaplacian operators 
e = ones(N,1);
L = 1/dx^4 * spdiags([e -4*e 6*e -4*e e], -2:2, N, N);
LHS = speye(N,N) + 0.5*dt*L;

% construct boundary condition matrices
L_bc_minus = 1/dx^4*[1, -4;
                     0,  1];
...
while (t < t_final)
  ...
  rhs(1:2) = rhs(1:2) - 0.5*dt*L_bc_minus*u_bc_minus_cur ...
                        - 0.5*dt*L_bc_minus*u_bc_minus_next;
  rhs(N-1:N) = rhs(N-1:N) - 0.5*dt*L_bc_plus*u_bc_plus_cur ...
