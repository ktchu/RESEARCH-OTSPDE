...
% Analyze Results
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
P_FE_OTS = polyfit(log(grid_sizes(1:4)),log(err_FE_OTS(1:4)),1);
...
P_comp_time_CN_4thOrder = polyfit(log(err_CN_4thOrder(4:6)), ...
                                  log(comp_time_CN_4thOrder(4:6)),1);
...
hold on;
plot(grid_sizes,err_CN_4thOrder, 'md', ...
     'MarkerSize',14, ...
     'MarkerFaceColor','m');
...
plot(grid_sizes,err_CN_4thOrder, 'md', ...
     'MarkerSize',14, ...
     'MarkerFaceColor','m');
order_str = sprintf('4th-Order\nCrank-Nicholson');
text(30,1e-7,order_str);
...
axis([10 100000 1e-8 1e0]);
set(gca, 'ytick', [1e-10 1e-8 1e-6 1e-4 1e-2 1e0]);
xlabel('N');
ylabel('L^\infty Error');
filename = sprintf('4th_order_parabolic_eqn_1d_error_vs_N_roundoff.%s', print_suffix);
format_str = sprintf('-d%s', print_format);
print([fig_dir, '/', filename], format_str);
...
annotation(f, 'arrow', [.78 .78], [.61 .52], ...
           'linewidth', 4, 'headstyle', 'plain');
...
hold on;
loglog(err_CN_4thOrder, comp_time_CN_4thOrder, 'md', ...
       'MarkerSize',14, ...
       'MarkerFaceColor','m');
order_str = sprintf('4th-Order Crank-Nicholson\nSlope = %1.2f', ...
                    comp_time_exp_CN_4thOrder);
text(1e-9,3e-4,order_str);
annotation(f, 'arrow', [.57 .60], [.27 .31], ...
           'linewidth', 4, 'headstyle', 'plain');
