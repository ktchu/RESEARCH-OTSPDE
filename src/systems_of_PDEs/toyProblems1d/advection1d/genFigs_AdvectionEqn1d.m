...
for i = 1:length(grid_sizes)
  ...
  err_FE_OTS(i) = norm([err_u; err_v],'inf');
  err_u = u_FE-u_exact;
  err_v = v_FE-v_exact;
  err_FE(i) = norm([err_u; err_v],'inf');
...
text(100,0.3,order_str);

axis([10 1000 1e-4 1]);
xlabel('N');
ylabel('L^\infty Error');
filename = sprintf('advection_eqn_1d_error_vs_N.%s', print_suffix);
...
filename = sprintf('advection_eqn_1d_FE_OTS_soln.%s', print_suffix);
format_str = sprintf('-d%s', print_format);
print([fig_dir, '/', filename], format_str);
...
filename = sprintf('advection_eqn_1d_FE_soln.%s', print_suffix);
format_str = sprintf('-d%s', print_format);
print([fig_dir, '/', filename], format_str);
