%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% solve4thOrderParabolicEqnForwardEulerOTS1d() computes the solutions of 
...
% check arguments
if (nargin < 3)
  error('solve4thOrderParabolicEqnForwardEulerOTS1d: missing arguments');
end
if (nargin < 4)
  debug_on = 0;
end
...
% construct 2nd-order Biaplacian operators (for correction term)
L_5pt = 1/dx^4 * spdiags([e -4*e 6*e -4*e e], -2:2, N, N);
L_5pt(1,:) = 0; L_5pt(2,:) = 0; L_5pt(3,:) = 0;
L_5pt(N,:) = 0; L_5pt(N-1,:) = 0; L_5pt(N-2,:) = 0;
...
while (t < t_final)
  ...
  f_t = 2500*pi^2 ...
      * ( -80000*sin(2*pi*x)*cos(10000*pi*t) ...
        - 128*pi^3*sin(2*pi*x)*sin(10000*pi*t) ...
        + (-4*pi^2+25*pi^4)*sin(5*pi*x)*exp(-100*pi^2*t) );

  % update solution
  u = u + dt*(-L*u + f) + 0.5*dt^2*(f_t - L_5pt*f); 
