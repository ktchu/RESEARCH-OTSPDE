%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% solveAdvDiffEqnForwardEulerOTS1d() computes the solutions of the 1d 
...
% approximations for both the advection and the diffusion terms.  The 
% optimal time step of dx^2/(6D) and correction term are used.
%
% USAGE:
%   function [u, u_exact, x] = solveAdvDiffEqnForwardEulerOTS1d( ...
%                                D, A, ...
%                                dx, ...
%                                t_final, ...
%                                debug_on)
...
% check arguments
if (nargin < 4)
  error('solveAdvDiffEqnForwardEulerOTS1d: missing arguments');
end
if (nargin < 5)
  debug_on = 0;
end
...
while (t < t_final)
  ...
  % update solution
  u = u + dt*(D*(L*u) + A*(G*u)) + 0.5*A^2*dt^2*(L*u); 
