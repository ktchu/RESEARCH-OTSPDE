...
% the domain is truncated to -1 < x < 1 and the boundary conditions are 
% taken to be u(-1) = 1 and u(1) = -1. 
% 
% Kevin T. Chu
% 2007 September
% 
...
% first-order upwind (suboptimal time step)
t = t_init;
u = u_init;
while (t < t_final)
...
% compute error
err = u-u_exact;
err_L1_upwind1 = sum(abs(err))*dx
err_L_inf_upwind1 = max((err))
